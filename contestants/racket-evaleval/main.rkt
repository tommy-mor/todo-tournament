...
;;
;; A todo app built with the evaleval pattern: forms embed signed Racket code
;; in hidden fields. On POST, the code is verified, interpolated, and eval'd.
;; The response is JavaScript that morphs the DOM via Idiomorph.
...
(define (rand-id)
  (define n (+ (random 89999999) 10000000))
  (number->string n))
...
(define (visible)
  ...
  (cond
    [(equal? f "active")    (filter (lambda (t) (not (hash-ref t 'done))) ts)]
    [(equal? f "completed") (filter (lambda (t) (hash-ref t 'done)) ts)]
    [else ts]))

(define (find-todo id)
  (findf (lambda (t) (equal? (hash-ref t 'id) id)) (unbox todos)))
...
(define (footer)
  (define ts (unbox todos))
  (list* 'footer (hasheq 'class "todo-footer" 'id "footer")
         (if (null? ts)
             '()
             (list
              (list 'span (hasheq 'class "todo-footer__count" 'id "count")
                    (format "~a items left"
                            (length (filter (lambda (t) (not (hash-ref t 'done))) ts))))
              (list* 'div (hasheq 'class "todo-filters" 'id "filters")
                     (map (lambda (pair)
                            (define label (car pair))
                            (define f (cdr pair))
                            (list 'form (hasheq 'method "post" 'style "display:contents")
                                  (server
                                   (begin
                                     (set-box! filt (~ f))
                                     (js (morph "#todo-list" (todo-list))
                                         (morph "#footer" (footer)))))
                                  (list 'button (hasheq 'type "submit" 'class "todo-filter__btn")
                                        label)))
...
(define (todo-item t)
  ...
  (list 'li (hasheq 'id (string-append "todo-" id)
              ...
              (server
               (begin
                 ...
                 (js (morph (~ (string-append "#todo-" id))
                     ...
                     (morph "#footer" (footer)))))
              (list 'input (hasheq 'type "checkbox" 'class "todo-item__toggle"
                                   'onchange "this.form.requestSubmit()"
                                   'checked done?)))
        ;; Text
        (list 'span (hasheq 'class (if done? "todo-item__text--done" ""))
              (hash-ref t 'text))
        ;; Delete button
        (list 'form (hasheq 'method "post" 'style "display:contents")
              (server
               (begin
                 (set-box! todos
                   (filter (lambda (t) (not (equal? (hash-ref t 'id) (~ id))))
                           (unbox todos)))
                 (js (remove-el (~ (string-append "#todo-" id)))
                     (morph "#footer" (footer)))))
              (list 'button (hasheq 'type "submit" 'class "todo-item__delete") "\u00d7"))))

(define (todo-list)
  (list* 'ul (hasheq 'id "todo-list" 'class "todo-list")
         (map todo-item (visible))))

(define (add-form)
  (list 'form (hasheq 'id "add-form" 'action "/" 'method "post")
        (server
        ...
        (list 'input (hasheq 'type "text" 'name "new-todo"
                             'class "todo-new-input" 'autofocus #t
                             'placeholder "What needs to be done?" 'autocomplete "off"))
        (list 'button (hasheq 'type "submit" 'style "display:none") "Add")))

(define (page)
  (list 'html '()
        (list 'head '()
              (list 'meta (hasheq 'charset "utf-8"))
              (list 'title '() "TodoMVC \u2014 Racket evaleval")
              (list 'link  (hasheq 'rel "stylesheet" 'href "/tournament.css"))
              (list 'script (hasheq 'src "/idiomorph.min.js") "")
              (list 'script (hasheq 'src "/evaleval.js" 'defer #t) ""))
        (list 'body '()
              (list 'div (hasheq 'class "todo-app")
                    (list 'h1 (hasheq 'class "todo-app__title") "todos")
                    (add-form)
                    (todo-list)
                    (footer)))))
...
(define (get-form-params req)
  (define raw-bindings (request-bindings/raw req))
  (for/hash ([b (in-list raw-bindings)]
             #:when (binding:form? b))
...
(define (handle-post req)
  ...
  (unless signed
    (error 'handle-post "missing evaleval-snippet"))
